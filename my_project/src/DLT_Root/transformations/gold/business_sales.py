...
# Creating a Matrialized View:
@dlt.table(
    name = "business_sales",
    comment = "Calculating and storing the total sales for each region."
)
def business_sales():
    ...
    df_agg = df_prun.groupBy("region", "category").agg(F.sum("total_amount").alias("total_sales"))
